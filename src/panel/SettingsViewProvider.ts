...
const SETTINGS: SettingDef[] = [
  ...
  {
    label: 'Self-Update',
    description: 'Update 3dm to the latest version',
    command: '3dmake.runSelfUpdate',
    icon: 'sync',
    a11yLabel: 'Self-update 3dm to the latest version.',
  },
  {
    label: 'Show Help',
...
export class SettingItem extends vscode.TreeItem {
  constructor(def: SettingDef, currentBinary?: string) {
...
export class SettingsViewProvider implements vscode.TreeDataProvider<SettingItem> {
  private _onDidChange = new vscode.EventEmitter<SettingItem | undefined | void>();
  ...
  getChildren(): SettingItem[] {
    ...
    return SETTINGS.map(s => new SettingItem(s, binary));
  }
}
