...
const ACTIONS: ActionDef[] = [
  {
    ...
    icon: 'folder-opened',
    a11yLabel: 'Select project file or directory. Keyboard shortcut: Ctrl+Shift+O.',
  ...
  },
  {
    ...
    icon: 'play',
    a11yLabel: 'Build STL from the current OpenSCAD project. Keyboard shortcut: Ctrl+Shift+B.',
  ...
  },
  {
    ...
    icon: 'eye',
    a11yLabel: 'Generate an SVG silhouette preview. Keyboard shortcut: Ctrl+Shift+P.',
  ...
  },
  {
    ...
    icon: 'terminal',
    a11yLabel: 'Run a custom 3dm command with arbitrary arguments. Keyboard shortcut: Ctrl+Shift+X.',
...
export class ActionItem extends vscode.TreeItem {
  constructor(def: ActionDef) {
    ...
    this.tooltip = def.shortcut
      ? `${def.label} (${def.shortcut})`
      : def.label;
    this.command = {
      command: def.command,
      title: def.label,
...
export class QuickActionsProvider implements vscode.TreeDataProvider<ActionItem> {
  private _onDidChange = new vscode.EventEmitter<ActionItem | undefined | void>();
