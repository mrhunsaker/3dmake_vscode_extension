...
export class ConfigManager {
  ...
  getDefaultModel(): string {
    return vscode.workspace.getConfiguration('3dmake').get<string>('defaultModel', '').trim();
  }

  getDefaultView(): string {
    return vscode.workspace.getConfiguration('3dmake').get<string>('defaultView', '3sil').trim();
  }

  getDefaultProfile(): string {
    return vscode.workspace.getConfiguration('3dmake').get<string>('defaultProfile', '').trim();
  }

  shouldShowOutputOnRun(): boolean {
    return vscode.workspace.getConfiguration('3dmake').get<boolean>('showOutputOnRun', true);
  }

  shouldAnnounceCompletion(): boolean {
    return vscode.workspace.getConfiguration('3dmake').get<boolean>('announceCommandCompletion', true);
  }
  ...
  getEffectiveBinaryPath(): string {
    ...
    const envPath = process.env['THREE_DM_PATH'] ?? process.env['THREEDM_PATH'] ?? '';
    if (envPath) {
      return envPath;
    }
  ...
  setModelOverride(v: string | undefined): void { this.modelOverride = v; }
  getModelOverride(): string | undefined { return this.modelOverride; }

  setViewOverride(v: string | undefined): void { this.viewOverride = v; }
  getViewOverride(): string | undefined { return this.viewOverride; }

  setProfileOverride(v: string | undefined): void { this.profileOverride = v; }
  getProfileOverride(): string | undefined { return this.profileOverride; }

  setOverlayOverride(v: string | undefined): void { this.overlayOverride = v; }
  getOverlayOverride(): string | undefined { return this.overlayOverride; }
  ...
  buildGlobalFlags(): string[] {
    ...
    if (view) { flags.push('-v', view); }
    if (profile) { flags.push('-p', profile); }
    if (overlay) { flags.push('-o', overlay); }
    return flags;
  }

  // ── SVG path tracking ─────────────────────────────────────────────

  setLastSvgPath(p: string): void { this.lastSvgPath = p; }
  getLastSvgPath(): string | undefined { return this.lastSvgPath; }
